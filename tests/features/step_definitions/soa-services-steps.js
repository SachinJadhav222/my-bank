...
const soaWorld = require('../support/soa-searvices-support');
...
const myStepDefinitionsWrapper = function () {
    ...
    this.When(/^I make a GET request to "(.*)"$/i, function (uri) {
        return soaWorld.httpGet(this.config.soaEndpoints[uri]);

    });

    this.When(/^I make a DELETE request to "(.*)"$/i, function (uri) {
        return soaWorld.httpDelete(this.config.soaEndpoints[uri]);
    });

    this.When(/^I make a POST request to "(.*)"$/i, function (uri) {
        return soaWorld.httpPost(this.config.soaEndpoints[uri]);
        //return soaWorld.httpPost(uri);
    ...
    this.Then(/^The response property "(.*)" should be "(.*)"$/i, function (path, expectedValue, callback) {
        const actualValue = soaWorld.getValue(path);
       assert.equal(actualValue, expectedValue, soaWorld.prettyPrintError(actualValue, expectedValue));
        callback();
    });

    this.Then(/^The response status code should be "(.*)"$/i, function (expectedValue, callback) {

       assert.equal(this.statusCode, expectedValue, soaWorld.prettyPrintError(this.statusCode, expectedValue));
        callback();
    });
};
