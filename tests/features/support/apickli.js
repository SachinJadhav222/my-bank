...
const getAssertionResult = function (success, expected, actual, apickliInstance) {
    return {
        ...
        actual,
        response: {
            statusCode: apickliInstance.getNewRespCode(),
            headers: apickliInstance.getNewRespHeader(),
            body: apickliInstance.getNewRespBody(),
        },
    };
};

function Apickli(scheme, domain, fixturesDirectory, variableChar) {
   console.log('This is APICKLI');
    this.domain = domain;
    this.headers = {};
    this.cookies = [];
    this.httpResponse = {};
    this.actualResponseBody = {};
    this.actualResponseStatusCode = 0;
    this.actualResponseHeaders = '';
    this.requestBody = '';
    ...
    // this.actualResponseBody=actRespBody;
    // this.actualResponseHeaders=actRespHead;
    // this.actualResponseStatusCode=actRespCode;
    // console.log('Actual Resp body---->>>>',this.actualResponseBody);
...
Apickli.prototype.getResponseObject = function () {
    // console.log('This is response 1--->',this.httpResponse.body)
    // const resp = JSON.parse(this.httpResponse.body);
    //console.log(respBody1)
    return this.httpResponse;
};

Apickli.prototype.getResponseBody = function () {
     console.log('This is response Body --->');
    // const respBody = JSON.parse(this.httpResponse.body);
    // console.log('Response Body------>',respBody)
    //return this.actualResponseBody;
    return getActRespBody();
};

Apickli.prototype.getResponseCode = function () {
     console.log('This is response Code--->');
    // const respStatusCode = this.httpResponse.statusCode;
    // console.log('Response Status code---->',respStatusCode)
    //return this.actualResponseStatusCode;
    return getActRespCode();
};
Apickli.prototype.getResponseHeaders = function () {
    console.log('This is response Headers--->');
    // const respHeaders = JSON.parse(this.httpResponse.headers);
    // console.log('Headers------>',respHeaders)
   // return this.actualResponseHeaders;
    return getActRespHead();
};
...
Apickli.prototype.setHeader = function (headerName, headerValue) {
    //const this=this;
...
Apickli.prototype.assertResponseCode = function (responseCode) {
    ...
    //responseCode=200;
    //const realResponse = this.getResponseObject().toString();
    //const realResponse = this.getResponseObject();
    // console.log('Response----1>>',realResponse);
    // const sc=this.getResponseCode();
    // console.log('Status code=========>>>',sc)
    // console.log('This is Response in the Assert Method===>',realResponse);
    // const realResponseCode = this.getResponseObject().statusCode.toString();
    const realResponseCode = this.getResponseCode();
...
Apickli.prototype.sendRequest = function (method, resource, callback) {
    ...
    request(options, function (error, response) {
        ...
        // getActRespCode();
        // getActRespHead();
        self.actualResponseBody = JSON.parse(response.body);
        self.actualResponseStatusCode = response.statusCode;
        self.actualResponseHeaders = response.headers;

        // console.log('This is response headers in Request ====>', self.actualResponseHeaders);
        // console.log('This is response status code in Request ====>', self.actualResponseStatusCode);
        // console.log('This is response body in Request ====>', self.actualResponseBody);

        // function setResponseBody(response,callback){
        //     console.log('Response in Set Method---->',response.body)
        //     actRespBody = JSON.parse(response.body);
        //     actRespCode = response.statusCode;
        //     actRespHead = response.headers;
        //     console.log('<<<<<<<<<<<<<<<-------------Response is Set ------------->>>>>>>>>>>>>>>>');
        //     callback;
        // };

        self.setNewResp(response);
...
Apickli.prototype.setNewResp=function(response) {
    this.httpResponse=response;
    console.log('SET RESP===>>>',this.httpResponse.body);
}

Apickli.prototype.getNewRespBody=function() {
    let respBody= JSON.parse(this.httpResponse.body);
    console.log('GET BODY===>>>',respBody);
    return respBody;

}
Apickli.prototype.getNewRespCode=function() {
    return this.httpResponse.statusCode;

}
Apickli.prototype.getNewRespHeader=function() {
    return this.httpResponse.headers;

}


function getActRespBody(){
    console.log('Resp Body in Get--->',actRespBody);
    return actRespBody;
};

function getActRespCode(){
    console.log('Resp Code in Get--->',actRespCode);
    return actRespCode;
};
function getActRespHead(){
    console.log('Resp Head in Get--->',actRespHead);
    return actRespHead;
}
