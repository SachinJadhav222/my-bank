@cmpcbo-521 @client @workflow @createClientMVP @regression @sprint11
Feature: CMPCBO-521 - Approver Colleague search workflow in pending status
  ...
  status on check and confirm page with the displayed workflow id, only the back to search button is visible.
    ...
    And the "workflow_approved_button" is visible
    And the "workflow_reject_button" is visible
    ...
    Examples:
      ...
      | Create a new client   | Pending | 8063077318  |
