@cmpcbo-480 @client @workflow @createClientMVP @regression @sprint11
Feature: CMPCBO-480 - Maker Colleague search workflow in approved status
  ...
  @cmpcbo480-01
  Scenario Outline: A Maker Colleague carried out a search workflow with approved status and confirmed the approved
  status on check and confirm page with the displayed workflow id, only the back to search button is visible.
    Given I navigate to the "approved_maker" page
    ...
    Examples:
      ...
      | Create a new client   | Approved | 2063077318  |
