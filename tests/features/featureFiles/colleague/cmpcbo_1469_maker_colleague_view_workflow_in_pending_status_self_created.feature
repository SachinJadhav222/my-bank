@cmpcbo-1469 @client @colleagueCreatePAUser @regression @sprint24
Feature: CMPCBO-1469 - Maker Colleague view workflow in Pending status self created
  ...
  Scenario Outline:  Approver search workflow created by a maker and sees; "Back to search", "Reject" and "Approve"
    ...
    Then the "pa_user_workflow_cancel_button" is visible
    Then the "pa_user_workflow_back_to_search_button" is visible
    And the "pa_user_workflow_reject_button" is not visible
    And the "pa_user_workflow_approve_button" is not visible
